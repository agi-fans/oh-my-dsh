...
完整的搜索优先级、`SKILL.md` 结构、stdio 与 HTTP 示例、环境变量展开、覆盖规则和当前协议限制，请参阅 [Skills 与 MCP](skills-and-mcp.zh-CN.md)。安装 DSH 命令、工具、提供方或 Auth bundle 是另一条计划中的 Profile 路径，见 [用户插件](plugins.zh-CN.md)。
