...
## Package layout
    ...
    packages/tui/omdsh-tui/        @omdsh/tui — composable TUI plugin suite
...
- Service definition — `@omdsh/tui/definition` declares the provider-neutral TUI protocol and view vocabulary.
- Local provider — `@omdsh/tui`/`provider-local` alone owns raw mode, key decoding, viewport, cursor and atomic differential rendering.
- Session runtime — `@omdsh/tui/session-runtime` owns the active Agent, persistence-backed create/resume/switch, model selection, projections and cleanup behind one API.
...
- Consumer — `@omdsh/tui/runner` only reads submitted input, routes slash commands through the session runtime, sends ordinary messages and handles exit.
...
- @omdsh/tui local provider, tool-presentation bridge, session runtime, human-interaction adapter, omdsh command contributions and thin runner
