...
| 能力 | 状态 | 实现位置 |
|---|---|---|
| 动态命令 | 已完成 | 挂载 `dsh-commands`；`SessionController` 将 agent-scoped command catalog 推给 TUI，未知命令不会误发给模型 |
...
| 能力 | 状态 | 实现位置 |
|---|---|---|
| Tool renderer registry | 已完成 | `tool-renderers.ts`，后注册的 exact-name renderer 可覆盖内置展示，异常时安全回退 |
| Coding tool 展示 | 已完成 | bash、文件读写/编辑、搜索、todo/goal/jobs、subagent、ask-user、plan 和通用 fallback |
...
| 多主题和状态栏 preset | 已完成 | dark、light、midnight、solarized、mono；minimal、compact、full 三档状态详情 |
...
- `SessionController` 是 Harness runtime 与 TUI 之间的会话控制模块；创建、恢复、模型、查询和命令调度不散落在键盘事件代码中。
- `TuiService` 是 presentation seam；终端 provider 不拥有 goal、compaction、approval、plan 等业务状态。
- Tool renderer 是贡献点；未识别工具仍有稳定通用展示。
...
当前 `@omdsh/tui` 有 27 个测试文件、249 个测试；TUI 与 app typecheck、完整组合 EOF 启动冒烟均通过。esbuild 对仓库既有 `es2024` target 会打印 warning，不影响测试结果。
