...
| 工作负载 | 诊断基线 | 当前中位数 | 提升 |
| --- | ---: | ---: | ---: |
| 恢复 10,000 轮对话 | 323.6 ms | 2.15 ms | 150.5× |
| 恢复 10,000 次工具调用 | 307.6 ms | 21.21 ms | 14.5× |
| 应用 10,000 次投影统计更新 | 81.5 ms | 0.44 ms | 185.2× |
| 在 5,000 轮对话上渲染 200 个缓存帧 | — | 总计 48.00 ms | 0.24 ms/帧 |
...
已经完成的对话区块会按照区块身份和渲染参数缓存格式化后的 Markdown 与工具行，稳定的对话正文则按照不可变区块数组缓存。编辑输入、更新状态、播放动画和滚动时都可以复用已经完成的内容，不需要重新格式化整段会话。

### 终端差分输出

每个帧都会按可见行与上一帧比较。终端写入器只重写发生变化的行，只清理失效的旧行，并保持目标光标位置。滚轮事件会被合并处理，所有宽度都按终端显示单元计算，因此 ANSI 样式、中文、Emoji 和组合字符不会因为布局错误触发额外的修正绘制。
...
该 Benchmark 会直接导入源码实现并刻意排除终端 I/O，适合发现算法复杂度回退，但绝对数值仍会随硬件、Node.js 版本、后台负载和运行时预热状态变化。

## 当前边界与后续方向

目前仍然可以测量到的增长来自正在流式生成的 Assistant Markdown 区块：文本追加后，后续语法可能改变此前内容的表现形式，因此活动区块需要重新参与格式化。在相同环境中，2,500 字符回复的一次更新约为 0.19 ms，5,000 字符约为 0.29 ms，极端的 25,000 字符约为 1.30 ms。这些数据仍低于当前帧预算，所以 omdsh 选择保留即时流式反馈，而不是引入肉眼可见的批处理延迟。

如果真实使用中的性能追踪表明响应已经超过这些范围，下一步候选方案是对流式 Delta 进行帧率限制合并，以及增量解析语法已经稳定的 Markdown 前缀。这些优化应由真实终端 Trace 驱动，而不是只依据微基准数字。
