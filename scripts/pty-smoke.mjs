...
// key — keyless), quits with Ctrl-D, and asserts a clean exit.
...
const term = pty.spawn(spawnCmd[0], spawnCmd[1], {
  ...
  rows: 30,
  cwd: root,
  env: { ...process.env, DEEPSEEK_API_KEY: 'sk-invalid-key-for-smoke', NO_COLOR: '1' },
})
...
const clean = out.replace(/\x1b\[[0-9;]*[A-Za-z]/g, '').replace(/\r/g, '')
const ok = exitCode === 0 && clean.includes('hi') && clean.includes('error:')
if (!ok) {
